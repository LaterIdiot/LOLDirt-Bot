	...
	async execute(message, args, db) {
		...
		if (!discordTag) {
			...
			if (!verifiedUser) {
				...
				await verified.insertOne(values).catch((err) => console.error(err));
			} else {
			...
			return message.channel.send(message.author, verificationSuccessEmbed);
		} else {
			...
			return message.channel.send(message.author, verificationFailureEmbed);
		}
	},
};
